...
export default function Proxy() {
  ...
  const columns = configColumns.concat({
    ...
    render: (_: unknown, record: GetProxyListResult) => (
      <Space>
        <TriggerModal renderModal={(renderProps) => <AddEditModal {...renderProps} title="编辑代理" handleUpdateList={handleUpdateList} id={record.id} />}>
  ...
  return (
    <div className="flex flex-col p-2 h-full 2xl:p-4">
      <div className="">
        <TriggerModal renderModal={(renderProps) => <AddEditModal {...renderProps} title="添加代理" handleUpdateList={handleUpdateList} />}>
